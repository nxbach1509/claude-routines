...
1. "thị trường căn hộ chung cư tuần này {date_str}" HOẶC "bất động sản nhà ở năm {year_str}"
2. "Savills CBRE JLL Vietnam residential market {year_str}"
3. "giá căn hộ TP.HCM Hà Nội {year_str}"
...
(2) Phân khúc có cơ hội tốt nhất {year_str}–{year_plus2_str}: [budget/lifestyle/luxury/wellness/\
...
• **Báo cáo CBRE/JLL/Savills Q1 {year_str}:** [Key findings nếu đã phát hành]
...
1. [Shift: Labor-cost advantage → Ecosystem advantage]
   Bằng chứng: [...] | Dự báo {year_str}–{year_plus5_str}: [...]
...
def build_prompt(session: dict, date_str: str) -> str:
    ...
    return template.format(
        ...
        date_str=date_str,
        year_str=year_str,
        year_plus2_str=str(year_int + 2),
        year_plus5_str=str(year_int + 5),
    )
