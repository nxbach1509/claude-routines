...
1. "thị trường căn hộ chung cư tuần này {date_str}" HOẶC "bất động sản nhà ở {year}"
2. "Savills CBRE JLL Vietnam residential market {year} latest"
3. "giá căn hộ TP.HCM Hà Nội {date_str}"
4. "tồn kho căn hộ absorption rate Vietnam {year}"
5. "lãi suất vay mua nhà ngân hàng Việt Nam mới nhất {year}"
6. "dự án bất động sản mới ra mắt TP.HCM {year}"
7. "Luật Đất đai 2024 nghị định thông tư mới nhất {year}"
8. "PropTech AI định giá bất động sản Việt Nam {year}"
9. "Build-to-Rent BTR affordable housing Asia {year}"
10. "McKinsey BCG housing market report {year}"
...
HƯỚNG DẪN TÌM KIẾM:
1. "Vietnam hotel occupancy ADR RevPAR {date_str} {year}"
2. "Savills CBRE JLL Vietnam hospitality market {year} latest"
3. "Vietnam tourism arrivals statistics {year} latest"
4. "serviced apartment Ho Chi Minh Hanoi occupancy rent {year}"
5. "condotel legal status Vietnam {year} update"
6. "Quyết định 26/2025 TPHCM căn hộ dịch vụ triển khai"
7. "integrated resort theme park Vietnam entertainment {year}"
8. "Airbnb short-term rental regulation Vietnam {year}"
9. "AI revenue management hospitality IHG Marriott {year}"
10. "sustainability green hotel certification Vietnam {year}"
11. "Vietnam international tourist arrivals {year}"
...
(2) Phân khúc có cơ hội tốt nhất trong 2 năm tới: [budget/lifestyle/luxury/wellness/\
...
HƯỚNG DẪN TÌM KIẾM:
1. "Vietnam Grade A office occupancy rent {date_str} {year} CBRE JLL Savills"
2. "office market Ho Chi Minh Hanoi H1 {year} net absorption"
3. "Vietnam retail mall occupancy rent {year}"
4. "co-working flexible office Vietnam expansion {year}"
5. "Vietnam CBD office sublease rate {year}"
6. "retail tenant mix F&B fashion entertainment mall Vietnam"
7. "hybrid work office demand Vietnam {year} badge data"
8. "green building LEED EDGE Vietnam office {year}"
9. "PCCC quy định phòng cháy chữa cháy tòa nhà {year}"
10. "experiential retail concept Asia case study {year}"
11. "CBRE JLL Savills office retail outlook Vietnam {year} report"
...
• **Experiential retail:** [Case study thành công mới nhất (Châu Á/Mỹ/EU) — \
tên concept + KPI cụ thể]
• **PropTech for office:** [Smart building / AI energy management / workplace \
analytics — ví dụ cụ thể + ROI]
• **Báo cáo CBRE/JLL/Savills gần nhất {year}:** [Key findings nếu đã phát hành]
...
HƯỚNG DẪN TÌM KIẾM:
1. "Vietnam industrial park occupancy rent FDI {date_str} {year}"
2. "Savills CBRE JLL Vietnam industrial market {year} latest"
3. "FDI Vietnam manufacturing registration {year} latest"
4. "Vietnam KCN ready-built factory rent Binh Duong Dong Nai Bac Ninh {year}"
5. "Vietnam data center capacity expansion {year} Viettel CMC Equinix"
6. "technology park semiconductor Vietnam {year}"
7. "China+1 supply chain Vietnam manufacturing FDI {year}"
8. "Vietnam vs Thailand Indonesia Malaysia industrial park cost comparison {year}"
9. "AI data center demand Vietnam hyperscaler AWS Azure Google {year}"
10. "global minimum tax Vietnam FDI impact {year}"
11. "green industrial park eco-industrial Vietnam ESG MNC tenant {year}"
12. "McKinsey future of Asia manufacturing report {year}"
13. "Vietnam semiconductor packaging testing assembly FDI {year}"
...
def build_prompt(session: dict, date_str: str) -> str:
    ...
    year = date_str.split("/")[-1]  # Extract year from DD/MM/YYYY format
