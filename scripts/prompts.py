...
1. "thị trường căn hộ chung cư tuần này {date_str}" HOẶC "bất động sản nhà ở tháng {month_str} {year_str}"
2. "Savills CBRE JLL Vietnam residential market {year_str}"
...
2. "Savills CBRE JLL Vietnam hospitality market {year_str}"
...
2. "office market Ho Chi Minh Hanoi {year_str} net absorption"
...
11. "CBRE JLL Savills office retail outlook Vietnam {year_str} report"
...
• **Experiential retail:** [Case study thành công mới nhất {year_str} (Châu Á/Mỹ/EU) — \
tên concept + KPI cụ thể]
• **PropTech for office:** [Smart building / AI energy management / workplace \
analytics — ví dụ cụ thể + ROI]
• **Báo cáo CBRE/JLL/Savills {year_str}:** [Key findings nếu đã phát hành]
...
2. "Savills CBRE JLL Vietnam industrial market {year_str}"
...
[Key findings mới nhất {year_str}]
...
def build_prompt(session: dict, date_str: str) -> str:
    ...
    year_str = parts[2] if len(parts) == 3 else "2026"
    month_str = parts[1] if len(parts) == 3 else "01"
