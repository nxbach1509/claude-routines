...
1. "thị trường căn hộ chung cư tuần này {date_str}" HOẶC "bất động sản nhà ở {month_year}"
2. "Savills CBRE JLL Vietnam residential market {quarter} {year}"
...
2. "Savills CBRE JLL Vietnam hospitality market {quarter} {year}"
...
(2) Phân khúc có cơ hội tốt nhất {year}–{year_plus2}: [budget/lifestyle/luxury/wellness/\
...
2. "office market Ho Chi Minh Hanoi {quarter} {year} net absorption"
...
11. "CBRE JLL Savills office retail outlook Vietnam {quarter} {year} report"
...
• **Experiential retail:** [Case study thành công mới nhất {year} (Châu Á/Mỹ/EU) — \
tên concept + KPI cụ thể]
• **PropTech for office:** [Smart building / AI energy management / workplace \
analytics — ví dụ cụ thể + ROI]
• **Báo cáo CBRE/JLL/Savills {quarter} {year}:** [Key findings nếu đã phát hành]
...
2. "Savills CBRE JLL Vietnam industrial market {quarter} {year}"
...
def build_prompt(session: dict, date_str: str) -> str:
    ...
    year = str(dt.year)
    year_plus2 = str(dt.year + 2)
    quarter = f"Q{(dt.month - 1) // 3 + 1}"
    month_year = f"{_MONTH_VI[dt.month - 1]} {year}"
