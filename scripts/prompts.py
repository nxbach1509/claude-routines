...
Bộ Xây dựng.
Năm hiện tại: {year_str} | Tháng hiện tại: {month_vi_str} | Quý: {quarter_str}\
"""
...
1. "thị trường căn hộ chung cư tuần này {date_str}" HOẶC "bất động sản nhà ở {month_vi_str} {year_str}"
2. "Savills CBRE JLL Vietnam residential market {quarter_str} {year_str}"
3. "giá căn hộ TP.HCM Hà Nội {month_vi_str} {year_str}"
...
7. "Luật Đất đai 2024 nghị định thông tư mới nhất {year_str}"
...
HƯỚNG DẪN TÌM KIẾM:
1. "Vietnam hotel occupancy ADR RevPAR {month_vi_str} {year_str}"
2. "Savills CBRE JLL Vietnam hospitality market {quarter_str} {year_str}"
...
6. "Quyết định 26/2025 TPHCM căn hộ dịch vụ triển khai {year_str}"
...
HƯỚNG DẪN TÌM KIẾM:
1. "Vietnam Grade A office occupancy rent {month_vi_str} {year_str} CBRE JLL Savills"
2. "office market Ho Chi Minh Hanoi {quarter_str} {year_str} net absorption"
...
6. "retail tenant mix F&B fashion entertainment mall Vietnam {year_str}"
...
11. "CBRE JLL Savills office retail outlook Vietnam {quarter_str} {year_str} report"
...
HƯỚNG DẪN TÌM KIẾM:
1. "Vietnam industrial park occupancy rent FDI {month_vi_str} {year_str}"
2. "Savills CBRE JLL Vietnam industrial market {quarter_str} {year_str}"
3. "FDI Vietnam manufacturing registration {year_str} latest"
4. "Vietnam KCN ready-built factory rent Binh Duong Dong Nai Bac Ninh {year_str}"
...
def build_prompt(session: dict, date_str: str, year_str: str, month_vi_str: str, quarter_str: str) -> str:
