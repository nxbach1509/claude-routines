...
def generate_report(date_str: str, week_num: int, date_from: str, date_to: str) -> str:
    """Call Claude with web-search enabled and return the finished report text."""
    client = anthropic.Anthropic(api_key=os.environ["ANTHROPIC_API_KEY"])
    prompt = build_ai_weekly_prompt(date_str, week_num, date_from, date_to)
...
def main() -> None:
    ...
    # Safety check: only run on Friday (weekday 4)
    if now.weekday() != 4:
    ...
    # Coverage period: last 7 days (Fri last week → Thu this week)
    date_from = (now - timedelta(days=7)).strftime("%d/%m/%Y")
    date_to = (now - timedelta(days=1)).strftime("%d/%m/%Y")

    log.info("AI Weekly Tuần %d: %s (kỳ %s → %s)", week_num, date_str, date_from, date_to)

    body = generate_report(date_str, week_num, date_from, date_to)
